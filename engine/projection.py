...
class Projection:
    ticker: str
    monthly_krw: float
    months: int                      # 총 개월 수. 연 단위만 받던 것을 개월로 바꿨다
...
def period_label(months: int) -> str:
    """개월 수를 '3년 6개월' 처럼 읽는 말로. 5년 단위로만 고르던 것을 풀면서 필요해졌다."""
...
def simulate(ticker: str, series: list, monthly_krw: float, months: int,
             step_months: int = 1, since: date | None = None) -> Projection:
    """series: [(date, adj_close), ...]  — 수정종가여야 총수익 기준이 된다.

    기간은 **총 개월 수**로 받는다. 예전에는 정수 연 단위만 받아 5년·10년처럼
    거친 선택만 가능했다.
    ...
    need = int(months)
    label = period_label(need)
    result = Projection(ticker=ticker, monthly_krw=monthly_krw, months=need,
    ...
    if len(points) < need + 1:
        have = len(points) / 12
        result.notes.append(
            f"{label} 시뮬레이션에는 {need + 1}개월치 시세가 필요한데 "
            f"{len(points)}개월치({have:.1f}년)뿐입니다. "
            f"기간을 {max(1, len(points) - 1)}개월 이하로 줄이거나 이력을 더 수집하세요.")
        return result
    ...
    if result.loss_windows:
        result.notes.append(
            f"{len(ordered)}번 중 {result.loss_windows}번은 {label}을 채우고도 "
            "원금을 밑돌았습니다.")
    if span < need * 2 / 12:
        result.notes.append(
